    ...
    db.query(sql, (err, results) => {
        ...
        const productSql = `
            SELECT 
                ...
                pn.product_name,
                pd.batch,
                pd.description AS db_description,
                pd.size,
                pd.hsncode,
                pd.gstrate,
                pd.listprice,
            ...
            LEFT JOIN product_details pd ON 
                p.item_code = pd.hsncode
                OR p.item_code = pd.batch
                OR p.item_description LIKE CONCAT('%', pd.description, '%')
                OR EXISTS (
                    SELECT 1 
                    FROM product_name pn2 
                    WHERE pn2.product_id = pd.product_id 
                    AND (p.item_description LIKE CONCAT('%', pn2.product_name, '%') OR pn2.product_name LIKE '%BALL PEIN%')
                )
                OR pd.size LIKE CONCAT('%', p.item_description REGEXP 'HEAD WEIGHT: *[0-9.]+ *(GMS|oz\.)|medium size', '%')
            LEFT JOIN main_category mc ON pd.maincategory_id = mc.maincategory_id
            LEFT JOIN sub_category sc ON pd.subcategory_id = sc.subcategory_id
            LEFT JOIN product_name pn ON pd.product_id = pn.product_id
        ...
        db.query(productSql, (err, productResults) => {
            ...
            const productsByLeadId = {};
            productResults.forEach(product => {
                const leadId = product.lead_id;
                if (!productsByLeadId[leadId]) productsByLeadId[leadId] = {};
                const productKey = [product.pr_no || '', product.item_code || '', product.item_description || ''].join('|');
                if (!productsByLeadId[leadId][productKey]) {
                    productsByLeadId[leadId][productKey] = {
                        email_product: {
                            unit: product.unit || null,
                            pr_no: product.pr_no || null,
                            pr_date: product.pr_date || null,
                            ...
                            uom: product.uom || null,
                            pr_quantity: product.pr_quantity || null,
                        },
                        matched_db_products: []
                    };
                }
                if (product.maincategory_name) {
                    productsByLeadId[leadId][productKey].matched_db_products.push({
                        maincategory_name: product.maincategory_name || null,
                        subcategory_name: product.subcategory_name || null,
                        product_name: product.product_name || null,
                        batch: product.batch || null,
                        description: product.db_description || null,
                        size: product.size || null,
            ...
            const formattedResults = results.map(contact => ({
                ...contact,
                products: Object.values(productsByLeadId[contact.id] || []).map(product => ({
                    email_product: product.email_product,
                    matched_db_products: product.matched_db_products,
                    is_ambiguous: product.matched_db_products.length > 1 && !product.email_product.item_code
                }))
            }));
